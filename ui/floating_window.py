...
class FloatingWindow(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.wave_progress >= 0.0:
            ...
            wave_gradient.setColorAt(0.0, QColor(200, 71, 255, int(20 * fade)))
            wave_gradient.setColorAt(max(0.0, (front - wave_width) / width), QColor(200, 71, 255, int(75 * fade)))
            wave_gradient.setColorAt(min(1.0, front / width), QColor(255, 71, 218, int(150 * fade)))
            wave_gradient.setColorAt(min(1.0, (front + 50.0) / width), QColor(255, 71, 126, 0))
            ...
            for y in range(0, height + 24, 24):
                ...
                edge = front + 28 * math.sin(normalized_y * 12.0 + self.wave_progress * 8.0)
                path.lineTo(edge, y)
            ...
            for glow_width, alpha in ((42, 18), (26, 28), (12, 48)):
                ...
                painter.setPen(QPen(QColor(255, 71, 126, int(alpha * fade)), glow_width))
        ...
        if self.scan_opacity > 0.0:
            ...
            glow_gradient.setColorAt(0.5, QColor(0, 255, 204, int(30 * opacity * pulse)))
            ...
            for line_width, alpha in ((12, 18), (4, 55), (1, 190)):
                painter.setPen(QPen(QColor(0, 255, 204, int(alpha * opacity * pulse)), line_width))
            ...
            bracket_pen = QPen(QColor(0, 255, 204, int(95 * opacity * pulse)), 1)
